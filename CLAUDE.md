...
7並べ (Sevens) — a browser-based Japanese card game built with Vue 3 + Vite (SPA) + TypeScript. One human player vs 3 CPU opponents, or online multiplayer. PWA-enabled with localStorage game state persistence.
...
- **`composables/useGame.ts`** — Single-player game composable; bridges pure game logic with Vue reactivity and drives the CPU turn loop
- **`composables/useOnlineGame.ts`** — Online multiplayer composable; manages WebSocket connection and game state synchronization
- **`composables/useJokerMode.ts`** — Joker placement mode logic
- **`composables/useModalKeyboard.ts`** — Modal keyboard navigation helper
- **`composables/useLocalStorage.ts`** — Persistence hook; game state is saved/restored from localStorage
- **`pages/index.vue`** — Landing page (home / room creation)
- **`pages/play.vue`** — Single-player game board
- **`pages/room/[id].vue`** — Online multiplayer room page
- **`components/game/`** — 12 focused components: `GameBoard`, `PlayerHand`, `HandCard`, `CpuPlayer`, `GameStatus`, `GameOverModal`, `ActionButtons`, `CardSlot`, `BoardRow`, `ResetConfirmModal`, `HomeConfirmModal`, `JokerReceivedOverlay`, `JokerComboPanel`
- **`components/online/`** — 4 online components: `Lobby`, `PlayerSeat`, `ShareModal`, `TurnTimer`
...
Dynamic suit colors (`blue`/`orange`/`violet`) are safelisted in `tailwind.config.ts` because they're generated at runtime. The `content` glob in `tailwind.config.ts` must include `./index.html` and `./app/**/*.{vue,ts}` to ensure all Tailwind utility classes are generated.
